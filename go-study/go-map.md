...
map 的设计也被称为 “The dictionary problem”（词典的问题），它的任务是设计一种数据结构用来维护一个集合的数据，并且可以同时对集合进行增删查改的操作。最主要的数据结构有两种：`哈希查找表（Hash table）`、`搜索树（Search tree）`。
...
hash table 是计算机中最重要的设计之一。大部分 hash table 实现了快速查找、添加、删除功能，Go 内置的 map 实现了这些功能。

## Go 中 map 的实现

Go 语言采用哈希查找表，并使用链表解决哈希冲突。
...
`B` 是 buckets 数组的长度的对数，也就是说，buckets 数组的长度就是 2^B。bucket 里面储存了 key 和 value。

buckets 是一个指针，最终指向一个结构体：
...
![](/home/user/workspace/notebook/go-study/go-map.assets/map-1.jpg)
...
bmap 是存放 k-v 的地方，仔细看 bmap 的内部组成。

![](/home/user/workspace/notebook/go-study/go-map.assets/map-2.jpg)
...
![](/home/user/workspace/notebook/go-study/go-map.assets/map-3.jpg)
...
![](/home/user/workspace/notebook/go-study/go-map.assets/map-4.jpg)
...
![](/home/user/workspace/notebook/go-study/go-map.assets/map-5.jpg)
...
![](./go-map.assets/map-6.jpg)

扩容完成后，overflow bucket 消失了，key 都集中到了一个 bucket，更为紧凑了，提高了查找的效率。

![](./go-map.assets/map-7.jpg)

假设触发了 2 倍的扩容，那么扩容完成后，老 buckets 中的 key 分裂到了 2 个 新的 bucket。一个在 x part，一个在 y 的 part。依据是 hash 的 lowbits。新 map 中 `0-3` 称为 x part，`4-7` 称为 y part。

![](./go-map.assets/map-8.jpg)
...
![](./go-map.assets/map-10.jpg)

这时，我们对此 map 进行遍历。假设经过初始化后，startBucket = 3，offset = 2。于是，遍历的起点将是 3 号 bucket 的 2 号 cell，下面这张图就是开始遍历时的状态：

![](./go-map.assets/map-11.jpg)
